...
public class NotificatorPushover extends Notificator {
    ...
    @Inject
    public NotificatorPushover(Config config, NotificationFormatter notificationFormatter, 
                              Client client, NotificatorCircuitBreaker circuitBreaker) {
    ...
    public void send(User user, NotificationMessage shortMessage, Event event, Position position) throws MessageException {
        // Use circuit breaker to protect against Pushover API failures
        circuitBreaker.executeNotification(this, user, shortMessage, event, position);
    }
    
    /**
     * Internal method to send notification through Pushover API.
     * This is called by the circuit breaker.
     */
    void sendInternal(User user, NotificationMessage shortMessage, Event event, Position position) throws MessageException {
        try {
