...
/**
 * MapmyIndia reverse geocoder implementation with circuit breaker, retry, and metrics support.
...
public class MapmyIndiaGeocoder extends JsonGeocoder {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(MapmyIndiaGeocoder.class);
    
    private final CircuitBreaker circuitBreaker;
    private final Retry retry;
    private final MeterRegistry meterRegistry;
    private final Timer requestTimer;
    private final String providerName = "mapmyindia";

    private static String formatUrl(String url, String key) {
        if (url == null) {
            url = "https://apis.mapmyindia.com/advancedmaps/v1/";
        }
        url += key + "/rev_geocode?lat=%f&lng=%f";
        return url;
    }

    /**
     * Creates a new MapmyIndiaGeocoder with resilience patterns and metrics.
     *
     * @param client JAX-RS client for HTTP requests
     * @param url Base URL for the MapmyIndia API (optional, defaults to official endpoint)
     * @param key MapmyIndia API license key
     ...
     */
    public MapmyIndiaGeocoder(Client client, String url, String key, int cacheSize, 
        ...
        eventPublisher.onStateTransition(event -> {
            LOGGER.info("MapmyIndia geocoder circuit breaker state changed from {} to {}", 
    ...
    public Address getAddress(double latitude, double longitude, ReverseGeocoderCallback callback) {
        ...
        Supplier<Address> addressSupplier = () -> {
            ...
            try {
                ...
                LOGGER.warn("Error getting address from MapmyIndia: {}", e.getMessage());
                throw e;
            }
        };
        ...
        if (callback != null) {
            ...
            CompletableFuture.supplyAsync(resilientSupplier)
                    ...
                    .exceptionally(e -> {
                        LOGGER.warn("Failed to get address from MapmyIndia", e);
    ...
    /**
     * Provides a fallback address when the MapmyIndia service is unavailable.
    ...
    public Address parseAddress(JsonObject json) {
        if (json.containsKey("responseCode") && json.getInt("responseCode") == 200) {
            ...
            if (results != null && !results.isEmpty()) {
                ...
                if (result.containsKey("houseNumber") && !result.isNull("houseNumber")) {
                    address.setHouse(result.getString("houseNumber"));
                }
                
                if (result.containsKey("houseName") && !result.isNull("houseName")) {
                    String houseName = result.getString("houseName");
                    if (!houseName.isEmpty()) {
                        if (address.getHouse() != null) {
                            address.setHouse(address.getHouse() + " " + houseName);
                        } else {
                ...
                if (result.containsKey("street") && !result.isNull("street")) {
                    address.setStreet(result.getString("street"));
                }
                
                if (result.containsKey("subLocality") && !result.isNull("subLocality")) {
                    address.setSuburb(result.getString("subLocality"));
                } else if (result.containsKey("subSubLocality") && !result.isNull("subSubLocality")) {
                    address.setSuburb(result.getString("subSubLocality"));
                }
                
                if (result.containsKey("locality") && !result.isNull("locality")) {
                    address.setSettlement(result.getString("locality"));
                } else if (result.containsKey("village") && !result.isNull("village")) {
                    address.setSettlement(result.getString("village"));
                }
                
                if (result.containsKey("subDistrict") && !result.isNull("subDistrict")) {
                    address.setDistrict(result.getString("subDistrict"));
                }
                
                if (result.containsKey("district") && !result.isNull("district")) {
                    address.setState(result.getString("district"));
                }
                
                if (result.containsKey("state") && !result.isNull("state")) {
                    address.setCountry(result.getString("state"));
                }
                
                if (result.containsKey("pincode") && !result.isNull("pincode")) {
                    address.setPostcode(result.getString("pincode"));
                }
                
                if (result.containsKey("formatted_address") && !result.isNull("formatted_address")) {
    ...
    protected String parseError(JsonObject json) {
        if (json.containsKey("responseCode") && json.getInt("responseCode") != 200) {
            return "Error response code: " + json.getInt("responseCode");
        }
        return null;
    }
