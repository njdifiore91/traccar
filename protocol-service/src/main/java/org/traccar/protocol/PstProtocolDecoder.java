...
/**
 * PST protocol decoder
 * Supports both direct decoding and message broker integration for microservices architecture
 */
public class PstProtocolDecoder extends BaseProtocolDecoder {
    ...
    public PstProtocolDecoder(Protocol protocol, MessageProducer messageProducer) {
        super(protocol);
    ...
    protected Object decode(
        ...
        if (type == MSG_STATUS) {
            ...
            // If message broker is available, publish the position
            if (messageProducer != null && messageProducer.isConnected() && position.getFixTime() != null) {
                messageProducer.publish("protocol.positions", position);
            }
